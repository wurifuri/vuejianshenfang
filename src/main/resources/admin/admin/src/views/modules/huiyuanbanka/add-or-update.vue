	...
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20220730/4b6f6d88fde34446904f3e1cd2807e39.png) center center/cover fixed no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="会员卡类型" prop="huiyuankaleixing">
					<el-select :disabled="ro.huiyuankaleixing" @change="huiyuankaleixingChange" v-model="ruleForm.huiyuankaleixing" placeholder="请选择会员卡类型">
						<el-option
							v-for="(item,index) in huiyuankaleixingOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.huiyuankaleixing" label="会员卡类型" prop="huiyuankaleixing">
					<el-input v-model="ruleForm.huiyuankaleixing" placeholder="会员卡类型" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="售价" prop="jine">
					<el-input v-model="ruleForm.jine" placeholder="售价" clearable  :readonly="ro.jine"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="售价" prop="jine">
					<el-input v-model="ruleForm.jine" placeholder="售价" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="有效天数" prop="youxiaotianshu">
					<el-input v-model="ruleForm.youxiaotianshu" placeholder="有效天数" clearable  :readonly="ro.youxiaotianshu"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="有效天数" prop="youxiaotianshu">
					<el-input v-model="ruleForm.youxiaotianshu" placeholder="有效天数" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="办卡时间" prop="bankashijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.bankashijian" 
						type="datetime"
						:readonly="ro.bankashijian"
						placeholder="办卡时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.bankashijian" label="办卡时间" prop="bankashijian">
					<el-input v-model="ruleForm.bankashijian" placeholder="办卡时间" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	...
	created() {
		this.ruleForm.bankashijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='youxiaotianshu'){
							this.ruleForm.youxiaotianshu = obj[o];
							this.ro.youxiaotianshu = true;
							continue;
						}
						if(o=='bankashijian'){
							this.ruleForm.bankashijian = obj[o];
							this.ro.bankashijian = true;
							continue;
						}
				}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huiyuanbanka/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huiyuanbankaCrossAddOrUpdateFlag = false;
						 this.parent.search();
